...
SENTIMENT_DIR = os.path.join(BASE_DIR, 'top_2_fill_comments')
BHASHINI_DIR = os.path.join(BASE_DIR, 'output', 'language_detection', 'bhashini', 'full_comments')
...
TEMPLATE_FILE = 'dashboard_template_v1_top2.html'
OUTPUT_FILE = os.path.join(BASE_DIR, 'ui', 'output', 'sentiment_analysis_report_top2.html')
...
def load_category_summaries(gemini_dir: str) -> dict:
    ...
    for summary_file in glob.glob(os.path.join(gemini_dir, '*_category_summary.json')):
        ...
        # strip _category_summary.json to get course_id
        course_id = basename.replace('_category_summary.json', '')
        try:
...
def analyze_dataset():
    ...
    for csv_file in csv_files:
        ...
        csv_path = os.path.join(BHASHINI_DIR, f"{course_id}.csv")
            ...
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if sent == 'positive':
                        ...
                        if issue and issue != 'Other Issue':
                            course_lang_buckets[lang_name]['issues'][issue] = \
                                course_lang_buckets[lang_name]['issues'].get(issue, 0) + 1
    ...
    if issue_counts:
        ...
        for k, v in sorted(issue_counts.items(), key=lambda x: x[1], reverse=True)[:15]:
            pct = (v / total_issues) * 100
            top_issues_list.append({'name': k, 'count': f"{v:,}", 'percent': f"{pct:.1f}"})

    # Overall strength breakdown
    top_strengths_list = []
    if strength_counts:
        total_strengths = sum(strength_counts.values()) or 1
        for k, v in sorted(strength_counts.items(), key=lambda x: x[1], reverse=True)[:15]:
    ...
    if language_counts:
        ...
        for k, v in sorted(language_counts.items(), key=lambda x: (CUSTOM_LANG_ORDER.get(x[0], 99), -x[1])):
            pct = (v / total_langs) * 100
            if pct > 20:
                skew_class, skew_label = 'dom', 'Dominant' if pct > 40 else 'High'
            elif pct > 5:
                skew_class, skew_label = 'bal', 'Moderate'
            else:
                skew_class, skew_label = 'min', 'Low'
    ...
    metrics = {
        ...
        'language_chips': [l['name'] for l in lang_summary],
...
def build_dashboard():
    print("Analyzing top-2 courses data...")
